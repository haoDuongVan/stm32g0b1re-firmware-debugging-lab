 ...
 */
#define BL_METADATA_FLASH_DOUBLEWORD_SIZE  8UL
#define BL_METADATA_FLASH_WRITE_SIZE       (((sizeof(BlMetadata_t) + \
                                              (BL_METADATA_FLASH_DOUBLEWORD_SIZE - 1UL)) / \
                                              BL_METADATA_FLASH_DOUBLEWORD_SIZE) * \
                                              BL_METADATA_FLASH_DOUBLEWORD_SIZE)
...
static uint32_t BlMetadata_GetFlashBank(uint32_t address)
{
#if defined(FLASH_DBANK_SUPPORT)
  /*
   * On dual-bank STM32 devices, Bank 2 starts at FLASH_BASE + FLASH_BANK_SIZE.
   * STM32G0B1RE is single-bank, so this branch is never taken at runtime.
   * The guard is here so the code compiles cleanly on dual-bank targets too.
   */
...
static uint32_t BlMetadata_GetFlashPage(uint32_t address)
{
#if defined(FLASH_DBANK_SUPPORT)
  if (address >= (FLASH_BASE + FLASH_BANK_SIZE)) {
...
uint8_t BlMetadata_Write(const BlMetadata_t *meta)
{
  ...
  uint8_t write_buffer[BL_METADATA_FLASH_WRITE_SIZE];
   ...
   */
  write_meta = *meta;
  write_meta.crc32 = BlMetadata_CalculateCrc(&write_meta);

  if (BlMetadata_IsValid(&write_meta) == 0U) {
    return 0U;
  }

  /*
   * Pad the write buffer to BL_METADATA_FLASH_WRITE_SIZE with 0xFF so that
  ...
  erase_init.Banks     = BlMetadata_GetFlashBank(BL_METADATA0_BASE_ADDR);
  erase_init.Page      = BlMetadata_GetFlashPage(BL_METADATA0_BASE_ADDR);
  erase_init.NbPages   = 1U;

  status = HAL_FLASHEx_Erase(&erase_init, &page_error);

  if (status == HAL_OK) {
    for (offset = 0UL; offset < sizeof(write_buffer); offset += BL_METADATA_FLASH_DOUBLEWORD_SIZE) {
